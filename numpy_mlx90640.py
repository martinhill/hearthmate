...
# eeData = [0] * 832
eeData = np.zeros(832, dtype=np.uint16)
I2C_READ_LEN = 2048
SCALEALPHA = 0.000001
...
class MLX90640:
    ...
    def __init__(self, i2c_bus: I2C, address: int = 0x33) -> None:
        ...
        # eeData_list = [0] * 832
        # self._I2CReadWords_legacy(0x2400, eeData_list)
        # print(eeData_list)
        self._I2CReadWords(0x2400, eeData)
        self.alpha = np.zeros(768, dtype=np.uint16)
        self.offset = np.zeros(768, dtype=np.int16)
        self.kta = np.zeros(768, dtype=np.uint16)
        self.kv = np.zeros(768, dtype=np.uint16)
        self._ExtractParameters()

    @property
    def serial_number(self) -> np.ndarray[np.uint16]:
        """3-item tuple of hex values that are unique to each MLX90640"""
        serialWords = np.zeros(3, dtype=np.uint16)
    ...
    def refresh_rate(self) -> int:
        ...
        controlRegister = np.zeros(1, dtype=np.uint16)
    ...
    def getFrame(self, framebuf: np.ndarray[np.float]) -> None:
        ...
        tr = 23.15
        # mlx90640Frame = [0] * 834
        mlx90640Frame = np.zeros(834, dtype=np.uint16)

        # initially accept either subpage
        subpage = None
        ...
        for _ in range(2):
            ...
            t2 = time.monotonic()
            vdd = self._GetVdd(mlx90640Frame)
            ta = self._GetTa(mlx90640Frame, vdd)
            tr = ta - OPENAIR_TA_SHIFT
            self._CalculateTo(mlx90640Frame, emissivity, vdd, ta, tr, framebuf)
            tend = time.monotonic()
            print("_GetFrameData duration =", t1-t0, "_CalculateTo duration =", tend-t2)
            subpage = 0 if status else 1

    def _GetFrameData(self, frameData: np.ndarray[np.uint16], subpage: int = None) -> int:
        dataReady = 0
        # statusRegister = [0]
        # controlRegister = [0]
        statusRegister = np.zeros(1, dtype=np.uint16)
        controlRegister = np.zeros(1, dtype=np.uint16)

        while dataReady == 0:
            self._I2CReadWords(0x8000, statusRegister)
            dataReady = statusRegister[0] & 0x0008
            page_measured = statusRegister[0] & 0x1
            if subpage is not None and subpage != page_measured:
                # Wrong subpage, so wait
                # print("wrong page:", page_measured)
                time.sleep(0.05)
                dataReady = 0
        ...
        # Clear new data available flag
        self._I2CWriteWord(0x8000, statusRegister[0] & 0xFFF7)
    ...
    def _GetTa(self, frameData: np.ndarray[np.uint16], vdd) -> float:
        # vdd = self._GetVdd(frameData)

        ptat = int(frameData[800])
        if ptat > 32767:
            ptat -= 65536

        ptatArt = int(frameData[768])
        if ptatArt > 32767:
            ptatArt -= 65536
    ...
    def _GetVdd(self, frameData: np.ndarray[np.uint16]) -> int:
        vdd = int(frameData[810])
        if vdd > 32767:
            vdd -= 65536
    ...
    def _CalculateTo(
        self, frameData: np.ndarray[np.uint16], emissivity: float, vdd: float, ta: float, tr: float, result: np.ndarray[np.float]
    ) -> None:  # noqa: PLR0914
        ...
        # Vector pixel calculations

        if mode == 0:
            pattern = self.ilPattern
        else:
            pattern = self.chessPattern

        patternvec = pattern == frameData[833]
        # print("patternvec =", patternvec, "frameData[833] =", frameData[833])
        pixel_data = frameData[:768][patternvec]
        ir_data = np.where(pixel_data[:768] > 32767, pixel_data - 65536, pixel_data)
        ir_data *= gain
        kta = self.kta[patternvec] / ktaScale
        kv = self.kv[patternvec] / kvScale
        ir_data -= self.offset[patternvec] * (1 + kta * (ta - 25)) * (1 + kv * (vdd - 3.3))

        if mode != self.calibrationModeEE:
            ir_data += (
                self.ilChessC[2] * (2 * self.ilPattern - 1)
                - self.ilChessC[1] * self.conversionPattern
            )

        ir_data -= self.tgc * irDataCP[subPage]
        ir_data /= emissivity

        alpha_compensated = SCALEALPHA * alphaScale / self.alpha[patternvec]
        ...
        Sx = (
            ...
            * alpha_compensated
            * alpha_compensated
            * (ir_data + alpha_compensated * taTr)
        )
        Sx = np.sqrt(np.sqrt(Sx)) * self.ksTo[1]

        To = np.sqrt(
                np.sqrt(
                    ir_data / (alpha_compensated * (1 - self.ksTo[1] * 273.15) + Sx) + taTr
                )
            ) - 273.15

        To_LT_ct1 = To < self.ct[1]
        To_LT_ct2 = To < self.ct[2]
        To_LT_ct3 = To < self.ct[3]

        alpha_corr_r_range = np.where(To_LT_ct1, alphaCorrR[0], 
            np.where(To_LT_ct2, alphaCorrR[1], 
            np.where(To_LT_ct3, alphaCorrR[2], alphaCorrR[3])))
        ksTo_range = np.where(To_LT_ct1, self.ksTo[0], 
            np.where(To_LT_ct2, self.ksTo[1], 
            np.where(To_LT_ct3, self.ksTo[2], self.ksTo[3])))
        ct_range = np.where(To_LT_ct1, self.ct[0], 
            np.where(To_LT_ct2, self.ct[1], 
            np.where(To_LT_ct3, self.ct[2], self.ct[3])))
        ...
        # TODO: remove bad pixels
        result[patternvec] = To
    ...
    def _ExtractParameters(self) -> None:
        ...
        #     if self.alpha[i] != self.alpha_np[i]:
        #         print("different alpha at ", i, ":", self.alpha[i], self.alpha_np[i])
    ...
    def _I2CWriteWord(self, writeAddress: int, data: int) -> None:
        ...
        cmd[2] = data >> 8
        cmd[3] = data & 0x00FF
        dataCheck = np.zeros(1, dtype=np.uint16)
    ...
    def _I2CReadWords(
        self, addr: int, buffer: Union[int, np.ndarray[np.uint16]], *, end: Optional[int] = None
    ) -> None:
        ...
        with self.i2c_device as i2c:
            while remainingWords:
                ...
                #     buffer[offset + i] = w
                outwords = np.frombuffer(inbuf, dtype=np.uint16, count=read_words)
                outwords.byteswap(inplace=True)
                buffer[offset:offset+read_words] = outwords
                offset += read_words
    ...
    def _ExtractPatterns(self):

        self.ilPattern = np.ndarray(
            [pixelNumber // 32 - (pixelNumber // 64) * 2 for pixelNumber in range(768)],
            dtype=np.int16)
        self.chessPattern = np.ndarray(
            [self.ilPattern[pixelNumber] ^ (pixelNumber - (pixelNumber // 2) * 2) for pixelNumber in range(768)],
            dtype=np.int16)

        self.conversionPattern = np.ndarray(
                [((pixelNumber + 2) // 4
                    - (pixelNumber + 3) // 4
                    + (pixelNumber + 1) // 4
                    - pixelNumber // 4) * (1 - 2 * self.ilPattern[pixelNumber]) for pixelNumber in range(768)],
            dtype=np.int16)
