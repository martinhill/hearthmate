...
class ThermalImageGenerator:
    ...
    def frame_to_rgb(self, frame: np.ndarray, colormap='ironbow') -> np.ndarray:
        ...
        normalized = (frame - min_temp) / temp_range
...
class HDF5Storage:
    ...
    def open(self):
        ...
        if 'mlx90640_frames' not in self.file:
            ...
            self.file.create_dataset('mlx90640_frames', 
                                   shape=(0, 24, 32), 
                                   maxshape=(None, 24, 32),
                                   dtype=np.float32,
                                   chunks=True)
            
            self.file.create_dataset('sequence_ids', 
                                   shape=(0,), 
                                   maxshape=(None,),
                                   dtype=np.uint32,
                                   chunks=True)
            
            self.file.create_dataset('timestamps_ms', 
                                   shape=(0,), 
                                   maxshape=(None,),
                                   dtype=np.uint32,
                                   chunks=True)
            
            self.file.create_dataset('vent_positions', 
                                   shape=(0,), 
                                   maxshape=(None,),
                                   dtype=np.float32,
                                   chunks=True)
            
            self.file.create_dataset('combustion_times', 
                                   shape=(0,), 
                                   maxshape=(None,),
                                   dtype=np.uint16,
                                   chunks=True)
    ...
    def store_packet(self, decoded_packet: dict):
        ...
        for dataset_name in ['mlx90640_frames', 'sequence_ids', 'timestamps_ms', 'vent_positions', 'combustion_times']:
...
class StoveLinkService:
    ...
    def __init__(self, mqtt_host: str, mqtt_port: int, input_topic: str, 
                 output_topic: str, hdf5_file: str, username: Optional[str] = None, 
        ...
        self.mqtt_client.on_connect = lambda client, userdata, connect_flags, reason_code, properties=None: self._on_connect(client, userdata, connect_flags, reason_code, properties)
        self.mqtt_client.on_message = lambda client, userdata, message, properties=None: self._on_message(client, userdata, message, properties)
        self.mqtt_client.on_disconnect = lambda client, userdata, disconnect_flags, reason_code, properties=None: self._on_disconnect(client, userdata, disconnect_flags, reason_code, properties)
...
def main():
    ...
    parser.add_argument('--input-topic', required=True, help='MQTT topic to subscribe for StoveLink data')
    parser.add_argument('--output-topic', required=True, help='MQTT topic to publish thermal images')
    parser.add_argument('--hdf5-file', required=True, help='HDF5 file for data storage')
    parser.add_argument('--diagnostic-topic', default=None, required=False, help='MQTT topic to pubish diagnostic info')
